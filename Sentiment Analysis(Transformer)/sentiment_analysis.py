...
if __name__ == '__main__':
    ...
    '''Train set'''
    # text = np.array(train_set['selected_text']).reshape((len(train_set['selected_text']), 1))
    ...
    # max_length = max_length + int(max_length*0.25)

    # Tokenizer
    tokenizer = keras.preprocessing.text.Tokenizer(num_words=1000)
    # Fit tokenizer(fit_on_text: will generate token id for each unique word)
    tokenizer.fit_on_texts(np.array(texts))
    # Text to sequence(texts_to_sequences: It will map the token id to its corresponding word)
    texts_to_int = tokenizer.texts_to_sequences(texts)
    # Pad sequence(add padding before the token ids)
    ...
    texts_to_int_pad = keras.preprocessing.sequence.pad_sequences(texts_to_int,
                                                                  ...
                                                                  truncating='pre',
                                                                  padding='pre')
    x_test = texts_to_int_pad
    y_test = label

    # Building Neural network
    model = keras.Sequential()
    model.add(layers.Input(shape=(max_length, )))
    model.add(TokenAndPositionEmbedding(max_length, vocab_size=1000, embed_dim=32))
